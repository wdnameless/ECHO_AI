...
Движок распознавания встроен в `.exe` и распаковывается сам. Модель **не
входит** в поставку: приложение больше не тащит внутрь себя 751 МБ. При первом
запуске в разделе **Настройки → Хранилище и модели** выберите модель — она
скачается с Hugging Face и будет проверена по SHA-256.

| Модель | Размер | WER en | Комментарий |
|---|---|---|---|
| Q4_K_M | 473 МБ | 3.30% | самая компактная |
| Q5_K_M | 534 МБ | 3.10% | близко к Q8, меньше места |
| Q6_K | 593 МБ | 3.08% | компромисс |
| **Q8_0** | **716 МБ** | **3.05%** | та же, что раньше была в установщике |
| F16 | 1.19 ГБ | 3.04% | без потерь по точности |
| F32 | 2.38 ГБ | 3.04% | полная точность |
