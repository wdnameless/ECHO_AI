...
3. **STT (Распознавание речи)**:
   - **Модель**: скачайте в разделе **Настройки → Хранилище и модели** (см. ниже).
...
## 📦 Портативная версия и модели

Из релиза можно взять **`Echo.AI_<версия>_portable_x64.zip`** — это один
исполняемый файл плюс маркер `.portable`. Установка не нужна: распакуйте в
любую папку (в том числе на флешку) и запустите.

```
Echo AI\
├── Echo AI.exe     ← всё приложение целиком
├── .portable       ← включает портативный режим
└── .echo-ai\       ← создаётся при первом запуске
    ├── engine\     ← движок распознавания, распакован из exe
    ├── models\     ← сюда скачиваются модели
    └── logs\
```

Движок распознавания встроен в `.exe` и распаковывается сам. Модели **не
входят** в поставку: приложение больше не тащит внутрь себя 751 МБ. При первом
запуске в разделе **Настройки → Модели распознавания речи** выберите модель —
она скачается с Hugging Face и будет проверена по SHA-256.

Доступны **71 модель в 15 семействах** (Whisper, Parakeet, Canary, Cohere,
Voxtral, Moonshine, Granite, Qwen3, GigaAM, SenseVoice, MedASR, Moss, Nemotron,
Fun-ASR, Breeze). Для каждой показаны точность, скорость, число языков и
опубликованный WER, а также все квантизации по отдельности. Ориентиры:

| Модель | Размер | Языков | Комментарий |
|---|---|---|---|
| Canary 180M Flash | 208 МБ | 4 | самая быстрая, для слабого железа |
| Nemotron 3.5 ASR 0.6B | 716 МБ | 28 | **рекомендуется**, есть русский |
| Parakeet TDT 0.6B v3 | 705 МБ | 25 | быстро и точно, европейские языки |
| Whisper Medium | 793 МБ | 99 | широчайшее покрытие языков |
| Cohere Transcribe | 2.3 ГБ | 14 | максимальная точность |

Каталог не написан руками: он генерируется из Hugging Face
(`scripts/gen_catalog.py`) — размеры и хеши берутся из листинга файлов,
метрики из карточек моделей, ревизии фиксируются по коммиту. Обновить список
...
Можно указать **свой** `.gguf` — движок принимает любой файл, а не только из
списка. Каталоги движка, моделей и логов задаются отдельно: например, модель на
большом диске, приложение — на системном.

История чатов и настройки остаются в каталоге пользователя (`%APPDATA%`) при
обычной установке и переносятся вместе с папкой в портативном режиме: база не
переезжает сама по себе, чтобы обновление не выглядело как потеря данных.
