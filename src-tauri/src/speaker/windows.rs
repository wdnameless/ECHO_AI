// Echo AI windows speaker input and stream
...
impl SpeakerInput {
    ...
    pub fn stream(self) -> SpeakerStream {
        ...
        let capture_thread = thread::spawn(move || {
            ...
            {
                error!("Echo AI Audio capture loop failed: {}", e);
            }
        });
        ...
        let actual_sample_rate = match init_rx.recv_timeout(Duration::from_secs(5)) {
            Ok(Ok(rate)) => rate,
            Ok(Err(e)) => {
                error!("Echo AI Audio initialization failed: {}", e);
                44100
            }
            Err(_) => {
                error!("Echo AI Audio initialization timeout");
                44100
            }
        };
