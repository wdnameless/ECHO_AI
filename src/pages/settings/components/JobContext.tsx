...
export function JobContext() {
  ...
  useEffect(() => {
    let mounted = true;
    (async () => {
      const stored = safeLocalStorage.getItem(STORAGE_KEYS.RAG_JOB_ENABLED);
      const ctx = await getRagContext("job");
      if (!mounted) return;
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      await setRagContext("job", text, file.name);
      setEnabled(true);
      safeLocalStorage.setItem(STORAGE_KEYS.RAG_JOB_ENABLED, "true");
    } catch {
  ...
  const handleSave = useCallback(async () => {
    setSaving(true);
    await setRagContext("job", content, sourceName ?? undefined);
    setSaving(false);
  }, [content, sourceName]);

  const handleDelete = useCallback(async () => {
    setSaving(true);
    await deleteRagContext("job");
    setContent("");
    setSourceName(null);
    safeLocalStorage.setItem(STORAGE_KEYS.RAG_JOB_ENABLED, "false");
    setEnabled(false);
    setSaving(false);
  }, []);

  return (
    <div id="job-context" className="space-y-3">
      <Header
        ...
        rightSlot={
          <div className="flex items-center gap-2">
            <Switch
              ...
              onCheckedChange={(v) => {
                setEnabled(v);
                safeLocalStorage.setItem(STORAGE_KEYS.RAG_JOB_ENABLED, String(v));
              }}
            />
      ...
      <Textarea
        ...
        placeholder="Paste the job description here or upload a .txt / .md / .pdf file..."
