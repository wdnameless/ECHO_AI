...
export const PluelyPrompts = () => {
  ...
  const fetchPluelyPrompts = async () => {
    ...
    try {
      ...
      console.error("Failed to fetch Echo AI prompts:", err);
      setError(
        typeof err === "string" ? err : "Failed to fetch Echo AI prompts"
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSelectPluelyPrompt = async (prompt: PluelyPrompt) => {
    ...
    try {
      ...
      // Save the selected Echo AI prompt to local storage for persistence
      ...
      console.error("Failed to select Echo AI prompt:", error);
    }
  };
    ...
    return (
      <div className="space-y-4 mt-6">
        ...
        <Empty
          ...
          description="Fetching Echo AI default prompts"
        />
      </div>
    );
  }
